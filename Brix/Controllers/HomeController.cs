...
namespace Brix.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(ILegoStoreRepository temp, IntexbrixContext context, ILogger<HomeController> logger, InferenceSession session)
        {
            _repo = temp;
            _context = context;
            _logger = logger;
            ...
            var fraud_dict = new Dictionary<int, string>
           {
            ...
            type_of_card
                ...
                var inputs = new List<NamedOnnxValue>
               {
                ...
                using (var results = _session.Run(inputs)) // takes the prediction with the inputs form the form whether fruad or not
                {
                    var prediction = results.FirstOrDefault(item => item.Name == "output_label")?.AsTensor<long>().ToArray();
                    if (prediction != null && prediction.Length > 0)
                    {
                        // Use the predictions to get the fraud or not fraud
                        var fraudType = fraud_dict.GetValueOrDefault((int)prediction[0], "Unknown");
